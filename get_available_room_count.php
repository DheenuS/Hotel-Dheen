...
function getBookedSingleRoomsCount($con)
{
    ...
    if ($result) {
        ...
        $count = $row['bookedSingleRoomsCount'];
        return $count;
    } else {
...
function updateAvailableRoomsCount($con, $count)
{
    $updateQuery = "UPDATE adminrooms SET availablerooms = ? WHERE id = 1";
...
$query = "SELECT totalrooms FROM adminrooms WHERE id = 1";
$result = mysqli_query($con, $query);
if ($result) {
    $row = mysqli_fetch_assoc($result);
    $totalRooms = $row['totalrooms'];
} else {
    $totalRooms = 0; // Handle query error for clarity (consider returning an error response)
}
...
$totalRooms = 10; // Set the total rooms count
$availableRoomCount = $totalRooms - $bookedSingleRoomsCount;

// Update the available rooms count in the adminrooms table
if (updateAvailableRoomsCount($con, $availableRoomCount)) {
    // Send the response
